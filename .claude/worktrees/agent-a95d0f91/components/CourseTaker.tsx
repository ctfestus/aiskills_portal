...
export function CourseTaker({
  ...
  const [phase, setPhase] = useState<'info' | 'otp' | 'course' | 'complete'>(
    collectStudentInfo ? 'info' : 'course'
  );
  ...
  // localStorage helpers — keyed by email so different students on same device don't share tokens
  const localTokenKey = (email: string) => `festforms_course_token_${email.trim().toLowerCase()}`;

  const saveLocalToken = (email: string, token: string) => {
    try { localStorage.setItem(localTokenKey(email), token); } catch { /* ignore */ }
  };

  const loadLocalToken = (email: string): string | null => {
    try {
      const token = localStorage.getItem(localTokenKey(email));
      if (!token) return null;
      // Decode payload client-side to check expiry (no secret needed, just expiry check)
      const [payload] = token.split('.');
      const { exp } = JSON.parse(atob(payload.replace(/-/g, '+').replace(/_/g, '/')));
      if (Date.now() > exp) {
        localStorage.removeItem(localTokenKey(email));
        return null;
      }
      return token;
    } catch { return null; }
  ...
  const clearLocalToken = (email: string) => {
    try { localStorage.removeItem(localTokenKey(email)); } catch { /* ignore */ }
  };
  ...
  const handleStartCourse = useCallback(async () => {
    ...
    // Check localStorage for an existing valid token — skip OTP if found
    const existingToken = loadLocalToken(studentEmail.trim());
    if (existingToken) {
    ...
    setOtpSending(true);
    const res = await fetch('/api/course-otp', {
      method: 'POST',
    ...
    if (!res.ok) {
      setAttemptError(resData.error || 'Failed to send verification code.');
      return;
    }
  ...
  const handleVerifyOtp = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/course-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'verify', email: studentEmail.trim(), code: otpCode }),
      });
      ...
      // Token obtained — save it before runCourseChecks so a subsequent
      // page load can recover even if the next step throws.
      sessionTokenRef.current = data.token;
      saveLocalToken(studentEmail.trim(), data.token);
      await runCourseChecks(studentEmail.trim().toLowerCase());
    } catch {
      // Network error or unexpected throw — token may already be saved in
      // localStorage so a refresh will recover, but tell the student to retry.
      setOtpError('Something went wrong. Please refresh and try again.');
    } finally {
      // Always unblock the button regardless of what happened above.
  ...
  const handleResendOtp = useCallback(async () => {
    ...
    if (!res.ok) {
      ...
      setOtpError(d.error || 'Failed to resend code. Please try again in a moment.');
    } else {
      setOtpResent(true);
  ...
  // Auto-start when pre-filled info arrives from the page modal
    ...
    return (
      ...
      >
        <div className="p-8 space-y-6">
          <div>
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl mb-4" style={{ background: `${accent}18` }}>
              ✉️
            </div>
            <h2 className={`text-xl font-semibold mb-1 ${textColor}`}>Check your email</h2>
            <p className={`text-sm ${mutedColor}`}>
              We sent a 6-digit code to <span className="font-semibold" style={{ color: accent }}>{studentEmail.trim()}</span>. Enter it below to continue.
            </p>
            <p className={`text-xs mt-2 flex items-center gap-1.5 ${mutedColor}`}>
              <span>📁</span> Can't find it? Check your <span className="font-semibold">spam or junk folder</span>.
            </p>
          </div>

          <div>
            <label className={`block text-xs font-medium mb-1.5 ${mutedColor}`}>Verification Code</label>
            <AnimatedField theme={config.theme || 'forest'} mode={config.mode || 'dark'}>
              <input
                ...
                placeholder="123456"
                className={`w-full bg-transparent border-none outline-none px-4 py-3 text-sm tracking-[0.3em] font-mono ${isDark ? 'text-white placeholder:text-zinc-600' : 'text-zinc-900 placeholder:text-zinc-400'}`}
                autoFocus
              />
            </AnimatedField>
          </div>
          ...
          <button
            ...
            style={{ background: accent }}
          >
            {otpVerifying ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Verify &amp; Start Course <ChevronRight className="w-4 h-4" /></>}
          </button>

          <div className="flex items-center justify-between">
            <button
              onClick={() => { setPhase('info'); setOtpCode(''); setOtpError(''); }}
              className={`text-xs font-medium transition-opacity hover:opacity-70 ${mutedColor}`}
            >
              ← Change email
            </button>
            <button
              onClick={handleResendOtp}
              disabled={otpSending || otpResent}
              className={`text-xs font-medium transition-opacity hover:opacity-70 ${otpResent ? 'text-green-500' : mutedColor}`}
            >
              {otpSending ? 'Sending…' : otpResent ? '✓ Code sent!' : 'Resend code'}
            </button>
          </div>
        </div>
      </motion.div>
  ...
  if (phase === 'info') {
    ...
    if (initialStudentName) {
            ...
            <div style={{ padding: '24px 28px 28px' }}>
              ...
              {checkingAttempts && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '20px 0' }}>
                  ...
                  <p className={`text-sm ${mutedColor}`}>Checking your records…</p>
                </div>
              )}
      ...
      if (checkingAttempts || existingCertId || showResumePrompt || attemptError) {
        ...
        <div className="p-8 space-y-6">
          ...
          {existingCertId ? (
            ...
            <button
              ...
              disabled={!studentName.trim() || !studentEmail.trim() || checkingAttempts || otpSending || !!attemptError}
              className="w-full py-3.5 rounded-xl font-semibold text-white flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed text-sm"
              style={{ background: accent }}
            >
              {(checkingAttempts || otpSending) ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Continue <ChevronRight className="w-4 h-4" /></>}
            </button>
          )}
        </div>
      </motion.div>
