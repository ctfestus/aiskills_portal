  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>, customData?: any) => {
    ...
    setAttendeeCount(c => c !== null ? c + 1 : 1);
      ...
      if (cfg.eventDetails?.isEvent) {
        ...
        if (data.passed === true && data.name && data.studentToken) {
          try {
            const certRes = await fetch('/api/course', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${data.studentToken}` },
              body: JSON.stringify({
  ...
  if (!form) {
    return (
      <div style={{ minHeight: '100vh', background: '#EEEAE3', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
        ...
        <h1 className="ff-pub" style={{ fontSize: 20, fontWeight: 700, color: '#111' }}>Form not found</h1>
        <p className="ff-pub" style={{ fontSize: 14, color: '#555' }}>This form has been deleted or doesn't exist.</p>
      ...
      <div className="ff-pub" style={{ minHeight: '100vh', background: t.page, position: 'relative', transition: 'background 0.3s' }}>
        ...
        <nav style={{ position: 'sticky', top: 0, zIndex: 30, background: t.nav, borderBottom: `1px solid ${t.navBorder}`, backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', height: 56, transition: 'background 0.3s' }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            <div style={{ width: 32, height: 32, borderRadius: 12, background: '#1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Star style={{ width: 16, height: 16, color: '#ADEE66', fill: '#ADEE66' }}/>
            </div>
            <span style={{ fontSize: 14, fontWeight: 700, color: t.logoText, transition: 'color 0.3s' }}>FestForms</span>
          </Link>
        ...
        <footer style={{ position: 'relative', zIndex: 10, textAlign: 'center', paddingBottom: 48, paddingTop: 16 }}>
          <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            ...
            <span style={{ fontSize: 12, color: t.footerText }}>Powered by <strong style={{ color: t.footerBold }}>FestForms</strong></span>
          </Link>
        </footer>
      </div>
    );
