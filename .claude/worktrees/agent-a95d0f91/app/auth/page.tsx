  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const [adminRes, { data: profile }] = await Promise.all([
          fetch('/api/admin', { headers: { Authorization: `Bearer ${data.session?.access_token}` } }),
          supabase.from('profiles').select('onboarding_completed').eq('id', data.session!.user.id).single(),
        ]);
        if (adminRes.ok) window.location.href = '/admin';
        else if (!profile?.onboarding_completed) window.location.href = '/onboarding';
        ...
        <div className="relative lg:w-[58%] bg-[#006128] flex flex-col overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col flex-1 px-8 pt-8 pb-10 lg:px-14 lg:pt-12 lg:pb-12">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-[#ADEE66] flex items-center justify-center shadow-lg">
                <svg viewBox="0 0 20 20" fill="none" className="w-5 h-5">
                  <rect x="3"  y="3"  width="6" height="6" rx="1.5" fill="#006128" />
                  <rect x="11" y="3"  width="6" height="6" rx="1.5" fill="#006128" />
                  <rect x="3"  y="11" width="6" height="6" rx="1.5" fill="#006128" />
                  <rect x="11" y="11" width="6" height="6" rx="1.5" fill="#006128" fillOpacity="0.45" />
                </svg>
              </div>
              <span className="text-white font-bold text-xl tracking-tight">FestForms</span>
            </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <h2 className="text-2xl font-bold text-[#006128] mb-1">
                    {isLogin ? 'Welcome back' : 'Join FestForms'}
                  </h2>
