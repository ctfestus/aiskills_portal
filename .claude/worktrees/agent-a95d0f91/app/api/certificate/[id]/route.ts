...
export async function GET(
  ...
  const [{ data: form }, { data: rawSettings }] = await Promise.all([
    adminSupabase.from('forms').select('title, config').eq('id', formId).single(),
    adminSupabase.from('certificate_settings').select('*').eq('form_id', formId).single(),
  ]);
