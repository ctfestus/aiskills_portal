...
import { getCreatorLimits } from '@/lib/subscription';
    ...
    if (type === 'blast') {
      ...
      if (limits.emails === 0) {
        return NextResponse.json(
          { error: 'Email broadcasts are not available on the free plan. Upgrade to Pro.' },
          { status: 403 }
        );
      }
      ...
      if (!isCron) {
        ...
        if (limits.emails === 0) {
          return NextResponse.json(
            { error: 'Email reminders are not available on the free plan. Upgrade to Pro.' },
            { status: 403 }
          );
        }
      ...
      if (typeof to === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to.trim()) && !data?.responseId) {
        ...
        if (limits.emails === 0) {
          return NextResponse.json(
            { error: 'Email features are not available on the free plan. Upgrade to Pro.' },
            { status: 403 }
          );
        }
