...
export async function GET(
  ...
  const fail = (reason: string) =>
    NextResponse.redirect(`${APP_URL}/settings?integration_error=${encodeURIComponent(reason)}`);
  ...
  return NextResponse.redirect(`${APP_URL}/settings?integration_success=${encodeURIComponent(provider)}`);
}
