...
export async function POST(req: NextRequest) {
  ...
  // Block free plan from all AI features
  const limits = await getCreatorLimits(jwt);
  if (limits.aiGenerations === 0) {
    return NextResponse.json(
      { error: 'AI features are not available on the free plan. Upgrade to Pro.' },
      { status: 403 }
    );
  }

  const { data: allowed, error: rlError } = await adminClient().rpc('check_ai_rate_limit', {
    p_user_id: userId, p_max: 20, p_window_ms: 60_000,
  });
  if (rlError) { console.error('[ai-course] rate limit:', rlError.message); }
  if (!allowed) {
...
- Do not include a sign-off from FestForms; write as the creator/host.`,
        config: {
          responseMimeType: 'application/json',
          responseSchema: {
            type: Type.OBJECT,
