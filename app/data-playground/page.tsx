  ...
  async function openPreview() {
    ...
    try {
      ...
      if (isZip) {
        ...
        const tables = await Promise.all(csvFiles.map(async name => ({ name: name.replace(/^.*\//, ''), content: await zip.files[name].async('string') })));
        setZipTables(tables);
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 sm:p-8" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(6px)' }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="w-full flex flex-col" style={{ maxWidth: 760, maxHeight: '94vh', background: C.card, borderRadius: 20, overflow: 'hidden', fontFamily: font, boxShadow: '0 32px 80px rgba(0,0,0,0.28)' }}>
        ...
        <div style={{ position: 'relative', height: 190, flexShrink: 0, background: C.input }}>
          ...
          <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(0,0,0,0.45)', border: 'none', borderRadius: '50%', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'white', backdropFilter: 'blur(4px)' }}>
            <X size={17} />
          </button>
        </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '22px 20px 28px' }}>
          ...
          {dataset.description && <p style={{ fontSize: 16, color: C.muted, lineHeight: 1.75, marginBottom: 28, marginTop: 0 }}>{dataset.description}</p>}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1100, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '12px' }}
          ...
          <div style={{ background: C.card, borderRadius: 16, width: '100%', maxWidth: 900, maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.35)', fontFamily: font }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: zipTables.length > 1 ? 12 : 18 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: 14, color: C.faint }}>{zipTables.length > 1 ? `${zipTables.length} tables - first 10 rows each` : 'First 10 rows preview'}</p>
                  </div>
                </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', padding: '12px 16px' }}>
              ...
              {!loadingPreview && (!preview || preview.length === 0) && <p style={{ fontSize: 14, color: C.faint, textAlign: 'center', padding: 40 }}>Preview not available for this file.</p>}
            </div>
          </div>
        </div>
      )}
