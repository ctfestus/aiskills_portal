...
export async function GET(req: NextRequest) {
  ...
  const [{ data: guides, error }, { data: instructors }] = await Promise.all([
    ...
    auth.supabase.from('students').select('id, full_name, avatar_url').in('role', ['instructor', 'admin']).order('full_name'),
  ]);
...
export async function POST(req: NextRequest) {
  ...
  const row = {
    ...
    full_name: fullName,
    profile_photo_url: clean(body?.profile_photo_url, 1000) || null,
...
export async function PATCH(req: NextRequest) {
  ...
  if (body?.profile_photo_url !== undefined) updates.profile_photo_url = clean(body.profile_photo_url, 1000) || null;
