...
import { Loader2, CheckCircle2, Zap, RotateCcw, BarChart3, Download } from 'lucide-react';
...
export interface PowerBILeanSubmission {
  submittedAt: string;
  overallScore: number;
  executiveSummary: string;
  issueTitles: string[];
...
interface Props {
  ...
  accentColor: string;
  completed: boolean;
  submissions?: PowerBILeanSubmission[];
  savedSummary?: PowerBILeanSubmission;
  context?: string;
  rubric?: string[];
  minScore?: number;
  maxReviews?: number;
  referenceImageUrl?: string;
  onComplete: (result: ReviewResult, lean: PowerBILeanSubmission, passed: boolean) => void;
}
...
export default function PowerBIReviewPlayer({ reqId, isDark, accentColor, completed, submissions = [], savedSummary, context, rubric, minScore, maxReviews, referenceImageUrl, onComplete }: Props) {
  ...
  async function handleSubmit() {
    if (!file) { setError('Please upload your Power BI file first.'); return; }
    setError('');
    setAnalyzing(true);
    try {
      ...
      if (referenceImageUrl) fd.append('referenceImageUrl', referenceImageUrl);

      const res = await fetch('/api/powerbi-review', {
        method: 'POST',
      ...
      setResult(json);
      const lean: PowerBILeanSubmission = {
        submittedAt: new Date().toISOString(),
        overallScore: json.overallScore,
        executiveSummary: json.executiveSummary ?? '',
        issueTitles: (json.issues ?? []).map((i: Issue) => i.title),
  ...
  async function downloadPdf() {
    ...
    try {
      await downloadReviewPdf(resultsRef.current, `powerbi-review-${Date.now()}.pdf`);
    } catch (err: any) {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: `${accentColor}10`, border: `1px solid ${accentColor}25` }}>
        ...
        <p className="text-sm font-medium" style={{ color: accentColor }}>
          Power BI review already submitted for this question.
        </p>
      </div>
    );
  }
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${border}` }}>
        <div className="px-5 py-4 flex items-start justify-between gap-4" style={{ background: '#0f172a' }}>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: 'rgba(255,255,255,0.4)' }}>AI Power BI Review</p>
    ...
    if (atLimit) {
      return (
        <div className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: isDark ? 'rgba(255,255,255,0.04)' : '#f8fafc', border: `1px solid ${border}` }}>
          <BarChart3 className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: muted }} />
          <div>
    ...
    return (
      <div className="space-y-3">
        ...
        <div
          ...
          <input ref={inputRef} type="file" accept=".pbix" className="hidden"
            onChange={e => { const f = e.target.files?.[0]; if (f) pickFile(f); }} />
          <BarChart3 className="w-8 h-8" style={{ color: file ? '#22c55e' : muted }} />
          {file
            ? <p style={{ fontSize: 13, fontWeight: 600, color: text }}>{file.name}</p>
            : <>
                <p style={{ fontSize: 13, fontWeight: 600, color: text }}>Drop your Power BI file here</p>
                <p style={{ fontSize: 12, color: muted }}>or click to browse - .pbix</p>
              </>
          }
        </div>
  ...
  return (
    <div ref={resultsRef} className="space-y-4" style={{ fontFamily: JB }}>
      ...
      <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '12px 20px', borderBottom: `1px solid ${border}` }}>
          <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.14em', color: muted }}>Layer Scores</p>
        </div>
        {result.layers.map((layer, i) => (
          <div key={layer.name} className="flex items-center gap-5"
            style={{ padding: '16px 20px', borderBottom: i < result.layers.length - 1 ? `1px solid ${border}` : 'none' }}>
            <div style={{ width: 36, flexShrink: 0, textAlign: 'center' }}>
              <span style={{ fontSize: 22, fontWeight: 900, color: scoreColor(layer.score), lineHeight: 1, display: 'block', fontVariantNumeric: 'tabular-nums' }}>{layer.score}</span>
              <span style={{ fontSize: 9, color: muted }}>/100</span>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="flex items-center justify-between mb-1.5">
                <p style={{ fontSize: 13, fontWeight: 700, color: text }}>{layer.name}</p>
                <span style={{ fontSize: 10, fontWeight: 700, color: scoreColor(layer.score), textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  {layer.score >= 80 ? 'Excellent' : layer.score >= 60 ? 'Good' : layer.score >= 40 ? 'Needs Work' : 'Critical'}
                </span>
              </div>
              <div style={{ height: 2, background: border, overflow: 'hidden', marginBottom: 8 }}>
                <div style={{ height: '100%', width: `${layer.score}%`, background: scoreColor(layer.score) }} />
              </div>
              <p style={{ fontSize: 12, color: muted, lineHeight: 1.5 }}>{layer.summary}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {minScore && result.overallScore < minScore ? (
        <div className="flex items-start gap-3 px-4 py-3" style={{ background: 'rgba(239,68,68,0.08)', borderLeft: '2px solid #ef4444' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#ef4444', opacity: 0.8 }}>You can try again with an improved file -- no point awarded until the minimum is reached.</p>
          </div>
        </div>
      ) : (
