...
server.tool(
  'list_cohorts',
  'List all cohorts. Use this to get cohort IDs before creating or assigning content.',
  {},
  async () => {
    const data = await supabaseQuery('cohorts', 'id, name, created_at');
    const formatted = data.map((c: any) => `• ${c.name} -- ID: ${c.id}`).join('\n');
...
server.tool(
  ...
  async ({ cohort_id }) => {
    const students = await supabaseRun(sb =>
      sb.from('students').select('id, full_name, email, role').eq('cohort_id', cohort_id).eq('role', 'student').order('full_name')
    ) as any[];
    if (!students?.length) return { content: [{ type: 'text', text: 'No students found in this cohort.' }] };
    const ids = students.map((s: any) => s.id);
    const xpRows = await supabaseRun(sb => sb.from('student_xp').select('student_id, total_xp').in('student_id', ids)) as any[];
    const xpMap: Record<string, number> = {};
    for (const x of xpRows ?? []) xpMap[x.student_id] = x.total_xp;
    const formatted = students.map((s: any, i: number) =>
      `${i + 1}. ${s.full_name} -- ${s.email} -- ${xpMap[s.id] ?? 0} XP`
    ).join('\n');
...
server.tool(
  ...
  async ({ cohort_id }) => {
    const courses = await supabaseRun(sb =>
      sb.from('courses').select('id, title').contains('cohort_ids', [cohort_id])
    ) as any[];
    if (!courses?.length) return { content: [{ type: 'text', text: 'No courses assigned to this cohort.' }] };

    const students = await supabaseRun(sb =>
      sb.from('students').select('id').eq('cohort_id', cohort_id).eq('role', 'student')
    ) as any[];
    const studentCount = students?.length ?? 0;
    if (!studentCount) return { content: [{ type: 'text', text: 'No students in this cohort.' }] };

    const courseIds = courses.map((c: any) => c.id);
    const attempts = await supabaseRun(sb =>
      sb.from('course_attempts').select('course_id, passed').not('completed_at', 'is', null).in('course_id', courseIds)
    ) as any[];

    const statsMap: Record<string, { completions: number; passes: number }> = {};
    for (const a of attempts ?? []) {
      if (!statsMap[a.course_id]) statsMap[a.course_id] = { completions: 0, passes: 0 };
      statsMap[a.course_id].completions++;
      if (a.passed) statsMap[a.course_id].passes++;
    }
...
server.tool(
  'get_leaderboard',
  'Get the top students by XP for a cohort.',
  {
    cohort_id: z.string().describe('Cohort ID -- if you only have a name, call list_cohorts first to resolve it'),
  ...
  async ({ cohort_id, limit = 10 }) => {
    const students = await supabaseRun(sb =>
      sb.from('students').select('id, full_name').eq('cohort_id', cohort_id).eq('role', 'student')
    ) as any[];
    if (!students?.length) return { content: [{ type: 'text', text: 'No students in this cohort.' }] };

    const ids = students.map((s: any) => s.id);
    const xpRows = await supabaseRun(sb =>
      sb.from('student_xp').select('student_id, total_xp').in('student_id', ids).order('total_xp', { ascending: false }).limit(limit)
    ) as any[];

    const nameMap: Record<string, string> = {};
    for (const s of students) nameMap[s.id] = s.full_name;

    const formatted = (xpRows ?? []).map((x: any, i: number) =>
      `${i + 1}. ${nameMap[x.student_id] ?? 'Unknown'} -- ${x.total_xp} XP`
    ).join('\n');
...
server.tool(
  ...
  async () => {
    ...
    const formatted = data.map((c: any) =>
      `• ${c.title} [${c.status}] -- ID: ${c.id} -- slug: ${c.slug}`
    ).join('\n');
...
server.tool(
  'get_course',
  'Get the full content of a course by ID, including all questions and lesson slides. Use this before updating a course to see what already exists.',
  {
  ...
  async ({ id }) => {
    const rows = await supabaseQuery(
      'courses',
      'id, title, description, status, slug, cohort_ids, questions, passmark, learn_outcomes, theme, mode, deadline_days',
    );
    const data = rows.find((r: any) => r.id === id);
    if (!data) return { content: [{ type: 'text', text: `Course not found: ${id}` }] };
...
server.tool(
  ...
  async ({ id, title, description, questions, cohort_ids, passmark, deadline_days, status, learn_outcomes, theme, mode, points_enabled, points_base }) => {
    ...
    const cur = await supabaseRun(sb =>
      sb.from('courses').select('title, description, cohort_ids, deadline_days, status, questions, passmark, learn_outcomes, theme, mode, points_enabled, points_base').eq('id', id).single()
    ) as any;
...
server.tool(
  'publish_content',
  'Publish or unpublish any course, virtual experience, or learning path by ID.',
  {
  ...
  async ({ id, content_type, status }) => {
    if (content_type === 'learning_path') {
      ...
      const rows = await supabaseQuery('learning_paths', 'id, title, item_ids, cohort_ids') as any[];
      const lp = rows.find((r: any) => r.id === id);
      if (!lp) return { content: [{ type: 'text', text: `Learning path not found: ${id}` }] };
...
server.tool(
  ...
  'List all virtual experiences (guided projects) created by the instructor.',
  {},
  async () => {
    const data = await supabaseQuery('virtual_experiences', 'id, title, status, slug, cohort_ids, industry, difficulty, created_at');
    const formatted = data.map((v: any) =>
      `• ${v.title} [${v.status}] -- ID: ${v.id} -- slug: ${v.slug}${v.industry ? ` -- ${v.industry}` : ''}`
    ).join('\n');
...
server.tool(
  ...
  'Get the full content of a virtual experience by ID, including all modules and lessons.',
  {
    id: z.string().describe('Virtual experience ID -- if you only have a name, call list_virtual_experiences first to resolve it'),
  },
  async ({ id }) => {
    const rows = await supabaseQuery('virtual_experiences', 'id, title, status, slug, cohort_ids, modules, learn_outcomes, industry, difficulty, role, company, duration, tools, tagline, theme, mode, deadline_days') as any[];
    const data = rows.find((r: any) => r.id === id);
    if (!data) return { content: [{ type: 'text', text: `Virtual experience not found: ${id}` }] };
    return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
  },
);
...
function wrapBody(raw: string | undefined): string {
  if (!raw) return '';
  const s = raw.trim();
  if (!s) return '';
  // Already contains HTML tags -- strip inline styles then return
  if (/<[a-z][\s\S]*>/i.test(s)) return stripInlineStyles(s);
...
function normalizeModules(modules: any[]): any[] {
...
async function uploadDataset(sb: any, dataset: any): Promise<any> {
  ...
  if (csvContent?.trim() && !meta.url) {
    ...
    const { error } = await sb.storage.from('datasets').upload(
      path, Buffer.from(csvContent, 'utf-8'), { contentType: 'text/csv', upsert: false },
    );
    if (!error) {
...
server.tool(
  ...
  'Create a new virtual experience (guided project / job simulation). Writes directly to the database -- supports large module content with no body-size limit. Returns the VE ID and slug.',
  {
  ...
  async ({ title, tagline, background, manager_name, manager_title, industry, difficulty, role, company, duration, tools, modules, dataset, learn_outcomes, cohort_ids, deadline_days, cover_image, status, theme, mode }) => {
    ...
    if (error) throw new Error(`Failed to create virtual experience: ${error.message}`);

    if ((cohort_ids ?? []).length > 0) {
      await sb.from('cohort_assignments').upsert(
        cohort_ids!.map(cohortId => ({ content_type: 'virtual_experience', content_id: ve.id, cohort_id: cohortId })),
        { onConflict: 'content_id,cohort_id', ignoreDuplicates: true },
      );
    }

    const totalLessons = normalizedMods.reduce((sum: number, m: any) => sum + (m.lessons?.length ?? 0), 0);
    const totalReqs    = normalizedMods.reduce((sum: number, m: any) =>
      sum + (m.lessons ?? []).reduce((s: number, l: any) => s + (l.requirements?.length ?? 0), 0), 0);
...
server.tool(
  ...
  'Update an existing virtual experience by ID. Writes directly to the database -- supports large module content with no body-size limit. Only fields you pass are updated; unset fields keep their current values.',
  {
  ...
  async ({ id, title, tagline, background, manager_name, manager_title, industry, difficulty, role, company, duration, tools, modules, dataset, learn_outcomes, cohort_ids, deadline_days, cover_image, status, theme, mode }) => {
    ...
    const { data: cur, error: fetchErr } = await sb
      ...
      .eq('user_id', userId)
      .single();
    if (fetchErr || !cur) throw new Error(`Virtual experience not found or not yours: ${id}`);
    ...
    const { error: updateErr } = await sb
      ...
      .eq('id', id)
      .eq('user_id', userId);
    if (updateErr) throw new Error(`Failed to update virtual experience: ${updateErr.message}`);

    // Upsert cohort_assignments for any newly added cohorts
    if (cohort_ids) {
      const oldCohorts: string[] = Array.isArray(cur.cohort_ids) ? cur.cohort_ids : [];
      const added = cohort_ids.filter(c => !oldCohorts.includes(c));
      if (added.length) {
        await sb.from('cohort_assignments').upsert(
          added.map(cohortId => ({ content_type: 'virtual_experience', content_id: id, cohort_id: cohortId })),
          { onConflict: 'content_id,cohort_id', ignoreDuplicates: true },
        );
      }
    }

    const totalLessons = finalMods.reduce((sum: number, m: any) => sum + (m.lessons?.length ?? 0), 0);
    const totalReqs    = finalMods.reduce((sum: number, m: any) =>
      sum + (m.lessons ?? []).reduce((s: number, l: any) => s + (l.requirements?.length ?? 0), 0), 0);
    return { content: [{ type: 'text', text: [
...
server.tool(
  'clone_virtual_experience',
  'Duplicate an existing virtual experience with a new title. All modules, lessons, requirements, dataset, and settings are copied. The clone is always created as a draft.',
  {
  ...
  async ({ id, title }) => {
    ...
    const { data: src, error: fetchErr } = await sb
      ...
      .eq('id', id)
      .single();
    if (fetchErr || !src) throw new Error(`Virtual experience not found: ${id}`);

    const base = title.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    const slug  = `${base}-${Math.random().toString(36).slice(2, 8)}`;

    // Re-generate IDs so lessons/requirements don't share IDs with the source
    const clonedMods = normalizeModules(
      (src.modules ?? []).map((m: any) => ({ ...m, id: undefined, lessons: (m.lessons ?? []).map((l: any) => ({ ...l, id: undefined, requirements: (l.requirements ?? []).map((r: any) => ({ ...r, id: undefined })) })) }))
    );
    ...
    if (error) throw new Error(`Failed to clone virtual experience: ${error.message}`);

    const totalLessons = clonedMods.reduce((sum: number, m: any) => sum + (m.lessons?.length ?? 0), 0);
    return { content: [{ type: 'text', text: [
...
server.tool(
  'list_learning_paths',
  'List all learning paths created by the instructor.',
  {},
  async () => {
    const data = await supabaseQuery('learning_paths', 'id, title, status, item_ids, cohort_ids, created_at');
    const formatted = data.map((p: any) =>
      `• ${p.title} [${p.status}] -- ID: ${p.id} -- ${p.item_ids?.length ?? 0} items`
    ).join('\n');
...
server.tool(
  'get_learning_path',
  'Get the full content of a learning path by ID, including all item IDs and assigned cohorts.',
  {
    id: z.string().describe('Learning path ID -- if you only have a name, call list_learning_paths first to resolve it'),
  },
  async ({ id }) => {
    const rows = await supabaseQuery('learning_paths', 'id, title, description, status, item_ids, cohort_ids, created_at') as any[];
    const data = rows.find((r: any) => r.id === id);
    if (!data) return { content: [{ type: 'text', text: `Learning path not found: ${id}` }] };
    return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
  },
);
...
server.tool(
  ...
  'Create a new learning path that groups courses and/or virtual experiences in order.',
  {
...
server.tool(
  ...
  async ({ id, title, description, item_ids, cohort_ids, status }) => {
    // Fetch current state so unset fields are not wiped
    const cur = await supabaseRun(sb =>
      sb.from('learning_paths').select('title, description, item_ids, cohort_ids, status').eq('id', id).single()
    ) as any;
...
async function bunnyGet(path: string) {
  if (!BUNNY_API_KEY || !BUNNY_LIBRARY_ID) throw new Error('BUNNY_API_KEY and BUNNY_LIBRARY_ID are required');
...
server.tool(
  ...
  'List all video collections (folders) in the Bunny.net video library. Use this to find the collection ID before importing videos.',
  {},
  async () => {
    const data = await bunnyGet('/collections?page=1&itemsPerPage=100&orderBy=name');
    const items = data.items ?? [];
    if (!items.length) return { content: [{ type: 'text', text: 'No collections found.' }] };
    const formatted = items.map((c: any) =>
      `• ${c.name} -- ID: ${c.guid} -- ${c.videoCount ?? 0} videos`
    ).join('\n');
...
server.tool(
  'list_bunny_videos',
  'List all videos in a Bunny.net collection (folder) in title order. Use this to preview video titles and order before creating a course.',
  {
  ...
  async ({ collection_id }) => {
    ...
    while (true) {
      ...
      if (allVideos.length >= (data.totalItems ?? 0) || items.length < 100) break;
      page++;
    }
    if (!allVideos.length) return { content: [{ type: 'text', text: 'No videos found in this collection.' }] };
    const sorted = allVideos.sort((a: any, b: any) =>
      (a.title ?? '').localeCompare(b.title ?? '', undefined, { numeric: true, sensitivity: 'base' })
    );
    const formatted = sorted.map((v: any, i: number) =>
      `${i + 1}. ${v.title} (${Math.round((v.length ?? 0) / 60)}m)`
    ).join('\n');
...
server.tool(
  ...
  'Fetch all videos from a Bunny.net collection (folder) in order and create a course. Each video becomes a lesson slide. If comprehension_questions are provided, they are interleaved after each video slide -- one entry per video in the same order.',
  {
  ...
  async ({ collection_id, title, description, cohort_ids, passmark, deadline_days, theme, mode, learn_outcomes, points_enabled, points_base, comprehension_questions }) => {
    ...
    const videos = allVideos.sort((a: any, b: any) =>
      (a.title ?? '').localeCompare(b.title ?? '', undefined, { numeric: true, sensitivity: 'base' })
    );
    if (!videos.length) return { content: [{ type: 'text', text: 'No videos found in this collection.' }] };
    ...
    const totalQ = slides.filter((s: any) => !s.lessonOnly).length;
    const summary = videos.map((v: any, i: number) => {
...
server.tool(
  'clone_course',
  'Duplicate an existing course with a new title. All questions, lessons, settings, and passmark are copied. The clone is created as a draft.',
  {
  ...
  async ({ id, title }) => {
    const cur = await supabaseRun(sb =>
      sb.from('courses').select('description, cohort_ids, deadline_days, questions, passmark, learn_outcomes, theme, mode, points_enabled, points_base').eq('id', id).single()
    ) as any;
...
server.tool(
  'send_announcement',
  'Create and publish an announcement to one or more cohorts. Appears in the student dashboard feed.',
  {
...
server.tool(
  'send_bulk_message',
  'Send a custom email to a segment of students. Use {{name}} in the message body to personalise each email. ALWAYS run with dry_run: true first to confirm recipient count before sending. Only set dry_run: false when the user explicitly confirms they want to send.',
  {
  ...
  async ({ subject, message_body, segment, cohort_id, form_id, dry_run = true }) => {
    if (dry_run) {
      ...
      const [coursesRaw, vesRaw] = await Promise.all([
        supabaseQuery('courses', 'id, title, cohort_ids') as Promise<any[]>,
        supabaseQuery('virtual_experiences', 'id, title, cohort_ids') as Promise<any[]>,
      ]);

      type Item = { id: string; cohort_ids: string[] };
      let content: Item[] = [
        ...((coursesRaw as any[]) ?? []),
        ...((vesRaw     as any[]) ?? []),
      ];
      if (form_id) content = content.filter(c => c.id === form_id);
      ...
      if (!activeCohortIds.length) return { content: [{ type: 'text', text: 'No cohorts found for this content.' }] };

      const students = await supabaseRun(sb =>
        sb.from('students').select('id, full_name, email, cohort_id').in('cohort_id', activeCohortIds).eq('role', 'student')
      ) as any[];
      if (!students?.length) return { content: [{ type: 'text', text: 'No students found.' }] };

      let recipients = students as any[];

      if (segment !== 'all') {
        const contentIds = content.map(c => c.id);
        const [courseAttempts, gpAttempts] = await Promise.all([
          supabaseRun(sb => sb.from('course_attempts').select('student_id, course_id, completed_at, updated_at').in('course_id', contentIds)) as Promise<any[]>,
          supabaseRun(sb => sb.from('guided_project_attempts').select('student_id, ve_id, completed_at, updated_at').in('ve_id', contentIds)) as Promise<any[]>,
        ]);

        const STALL_DAYS = 7;
        const attemptMap = new Map<string, { completed: boolean; lastActive: string }>();
        ...
        for (const item of content) {
          const itemStudents = students.filter((s: any) =>
            (Array.isArray(item.cohort_ids) ? item.cohort_ids : []).includes(s.cohort_id)
          );
      ...
      if (!recipients.length) return { content: [{ type: 'text', text: `No students match segment "${segment}".` }] };

      const sample = recipients.slice(0, 5).map((r: any) => `${r.full_name ?? 'Unknown'} <${r.email}>`).join('\n  ');
...
server.tool(
  'nudge_cohort',
  'Send motivational nudge emails to students who have not started or are stalled on a course. Only contacts students matching those statuses.',
  {
  ...
  async ({ course_id, cohort_id, status }) => {
    ...
    const students = await supabaseRun(sb =>
      sb.from('students').select('id, full_name, email').eq('cohort_id', cohort_id).eq('role', 'student')
    ) as any[];
    if (!students?.length) return { content: [{ type: 'text', text: 'No students in this cohort.' }] };

    // Get attempts for this course
    const attempts = await supabaseRun(sb =>
      sb.from('course_attempts').select('student_id, completed_at, updated_at').eq('course_id', course_id)
    ) as any[];
    ...
    const targets = students.filter((s: any) => {
      ...
      if (status === 'not_started') return !attempt;
      if (status === 'stalled') {
    ...
    if (!targets.length) return { content: [{ type: 'text', text: `No ${status.replace('_', ' ')} students found for this course.` }] };
    ...
    const recentNudges = await supabaseRun(sb =>
      sb.from('sent_nudges').select('student_id')
    ...
    const recentlyNudged = new Set((recentNudges ?? []).map((n: any) => n.student_id));
    const eligible = targets.filter((s: any) => !recentlyNudged.has(s.id));
    ...
    if (skipped) parts.push(`${skipped} skipped (nudged within the last ${NUDGE_COOLDOWN_DAYS} days).`);
...
server.tool(
  ...
  'List students in a cohort who have not started or are stalled (no activity in 7+ days) on any assigned course.',
  {
    cohort_id: z.string().describe('Cohort ID -- call list_cohorts to resolve name'),
  },
  async ({ cohort_id }) => {
    ...
    const attempts = await supabaseRun(sb =>
      sb.from('course_attempts').select('student_id, course_id, completed_at, updated_at')
    ...
    for (const student of students) {
      ...
      for (const course of courses) {
        const courseAttempts = studentAttempts.filter((a: any) => a.course_id === course.id);
        if (!courseAttempts.length) {
          notStarted.push(course.title);
          continue;
        }
        const completed = courseAttempts.some((a: any) => a.completed_at);
        if (completed) continue;
        const lastActive = courseAttempts.reduce((latest: string, a: any) =>
          a.updated_at > latest ? a.updated_at : latest, '');
        const days = Math.floor((Date.now() - new Date(lastActive).getTime()) / 86400000);
        if (days >= STALL_DAYS) stalled.push(`${course.title} (${days}d ago)`);
      }

      if (notStarted.length || stalled.length) {
        lines.push(`\n${student.full_name} <${student.email}>`);
        if (notStarted.length) lines.push(`  Not started: ${notStarted.join(', ')}`);
        if (stalled.length)    lines.push(`  Stalled: ${stalled.join(', ')}`);
      }
    }

    if (!lines.length) return { content: [{ type: 'text', text: 'No at-risk students found.' }] };
...
server.tool(
  'create_assignment',
  'Create a new assignment and assign it to one or more cohorts.',
  {
  ...
  async ({ title, scenario, brief, tasks, requirements, submission_instructions, related_course, cohort_ids, status }) => {
    ...
    const data = await supabaseRun(s =>
      s.from('assignments').insert({
        title:                   title.trim(),
...
server.tool(
  'list_assignments',
  'List all assignments created by the instructor.',
  {},
  async () => {
    const data = await supabaseQuery('assignments', 'id, title, status, cohort_ids, related_course, created_at');
    const formatted = data.map((a: any) =>
      `• ${a.title} [${a.status}] -- ID: ${a.id}${a.related_course ? ` -- course: ${a.related_course}` : ''}`
    ).join('\n');
...
server.tool(
  ...
  'List all ungraded assignment submissions. Use this to see what needs reviewing.',
  {
  ...
  async ({ assignment_id }) => {
    ...
    if (!rows?.length) return { content: [{ type: 'text', text: 'No pending submissions.' }] };

    const formatted = rows.map((r: any) => {
      const name  = (r.students as any)?.full_name ?? 'Unknown';
      const email = (r.students as any)?.email ?? '';
      const asgn  = (r.assignments as any)?.title ?? assignment_id ?? r.assignment_id;
      const date  = r.submitted_at ? new Date(r.submitted_at).toLocaleDateString() : 'unknown date';
      const preview = r.text_response ? `\n    "${(r.text_response as string).slice(0, 120)}..."` : '';
...
server.tool(
  'grade_submission',
  'Grade a student assignment submission with a score and optional feedback.',
  {
  ...
  async ({ submission_id, grade, feedback }) => {
    await supabaseRun(sb =>
      sb.from('assignment_submissions').update({
        grade,
...
server.tool(
  'get_student_report',
  'Get a full profile for a student: XP, course completions, pass rates, assignments, certificates.',
  {
    student_email: z.string().describe('Student email address'),
  },
  async ({ student_email }) => {
    const students = await supabaseRun(sb =>
      sb.from('students').select('id, full_name, email, cohort_id, role, created_at').eq('email', student_email.trim().toLowerCase())
    ) as any[];
    const student = students?.[0];
    if (!student) return { content: [{ type: 'text', text: `No student found with email: ${student_email}` }] };

    const [xpRows, attempts, certs, submissions, cohortRows] = await Promise.all([
      supabaseRun(sb => sb.from('student_xp').select('total_xp').eq('student_id', student.id)) as Promise<any[]>,
      supabaseRun(sb => sb.from('course_attempts').select('course_id, completed_at, passed, score').eq('student_id', student.id).not('completed_at', 'is', null).order('completed_at', { ascending: false })) as Promise<any[]>,
      supabaseRun(sb => sb.from('certificates').select('id, course_id, issued_at').eq('student_id', student.id).eq('revoked', false)) as Promise<any[]>,
      supabaseRun(sb => sb.from('assignment_submissions').select('assignment_id, grade, graded_at, submitted_at').eq('student_id', student.id).not('submitted_at', 'is', null)) as Promise<any[]>,
      supabaseRun(sb => sb.from('cohorts').select('id, name').eq('id', student.cohort_id)) as Promise<any[]>,
    ]);

    const xp     = (xpRows as any[])?.[0]?.total_xp ?? 0;
    const cohort = (cohortRows as any[])?.[0]?.name ?? student.cohort_id ?? 'Unknown';

    const completedAttempts = (attempts as any[]) ?? [];
    const totalCompleted = completedAttempts.length;
    const totalPassed    = completedAttempts.filter((a: any) => a.passed).length;
    const avgScore       = totalCompleted
      ? Math.round(completedAttempts.reduce((sum: number, a: any) => sum + (a.score ?? 0), 0) / totalCompleted)
      : 0;

    const certsCount  = (certs as any[])?.length ?? 0;
    const submCount   = (submissions as any[])?.length ?? 0;
    const gradedCount = (submissions as any[])?.filter((s: any) => s.graded_at).length ?? 0;
    const avgGrade    = gradedCount
      ? Math.round((submissions as any[]).filter((s: any) => s.graded_at && s.grade != null).reduce((sum: number, s: any) => sum + s.grade, 0) / gradedCount)
      : null;
    ...
    if (completedAttempts.length) {
      const courseIds = [...new Set(completedAttempts.map((a: any) => a.course_id))];
      const courseRows = await supabaseRun(sb => sb.from('courses').select('id, title').in('id', courseIds)) as any[];
      ...
      if (completedAttempts.length > 10) lines.push(`  ... and ${completedAttempts.length - 10} more`);
    }
...
server.tool(
  ...
  'Get detailed analytics for a course: attempt count, completion rate, pass rate, average score, top scorers, and students who failed multiple times.',
  {
    course_id: z.string().describe('Course ID -- call list_courses to resolve name'),
  },
  async ({ course_id }) => {
    const [courseRows, attempts] = await Promise.all([
      supabaseRun(sb => sb.from('courses').select('title, passmark, cohort_ids').eq('id', course_id).single()) as Promise<any>,
      supabaseRun(sb => sb.from('course_attempts').select('student_id, completed_at, passed, score').eq('course_id', course_id)) as Promise<any[]>,
    ]);

    const course = courseRows as any;
    if (!course?.title) return { content: [{ type: 'text', text: `Course not found: ${course_id}` }] };

    const allAttempts   = (attempts as any[]) ?? [];
    const completed     = allAttempts.filter((a: any) => a.completed_at);
    const inProgress    = allAttempts.filter((a: any) => !a.completed_at);
    const passed        = completed.filter((a: any) => a.passed);
    const failed        = completed.filter((a: any) => !a.passed);
    const uniqueStudents = new Set(allAttempts.map((a: any) => a.student_id)).size;
    const avgScore = completed.length
      ? Math.round(completed.reduce((s: number, a: any) => s + (a.score ?? 0), 0) / completed.length)
      : 0;

    // Students who failed 2+ times with no pass
    const studentAttempts: Record<string, { fails: number; passed: boolean }> = {};
    for (const a of completed) {
      if (!studentAttempts[a.student_id]) studentAttempts[a.student_id] = { fails: 0, passed: false };
      if (a.passed) studentAttempts[a.student_id].passed = true;
      else studentAttempts[a.student_id].fails++;
    }
    ...
    const topStudentRows = studentIds.length
      ? await supabaseRun(sb => sb.from('students').select('id, full_name').in('id',
          [...completed].sort((a: any, b: any) => (b.score ?? 0) - (a.score ?? 0)).slice(0, 5).map((a: any) => a.student_id)
        )) as any[]
      : [];
...
server.tool(
  ...
  'Analyze a cohort across all assigned courses. Returns completion rates, pass rates, engagement gaps, and flags underperforming content. Use this to spot where students are struggling and what action to take.',
  {
    cohort_id: z.string().describe('Cohort ID -- call list_cohorts to resolve name'),
  },
  async ({ cohort_id }) => {
    const [students, courses] = await Promise.all([
      supabaseRun(sb => sb.from('students').select('id').eq('cohort_id', cohort_id).eq('role', 'student')) as Promise<any[]>,
      supabaseRun(sb => sb.from('courses').select('id, title, passmark').contains('cohort_ids', [cohort_id])) as Promise<any[]>,
    ]);

    const studentCount = (students as any[])?.length ?? 0;
    if (!studentCount) return { content: [{ type: 'text', text: 'No students in this cohort.' }] };
    if (!(courses as any[])?.length) return { content: [{ type: 'text', text: 'No courses assigned to this cohort.' }] };

    const courseIds = (courses as any[]).map((c: any) => c.id);
    const studentIds = (students as any[]).map((s: any) => s.id);

    const attempts = await supabaseRun(sb =>
      sb.from('course_attempts').select('student_id, course_id, completed_at, passed, score')
        .in('course_id', courseIds).in('student_id', studentIds)
    ) as any[];

    const lines: string[] = [`Cohort performance -- ${studentCount} students\n`];

    const flags: string[] = [];

    for (const course of courses as any[]) {
      const courseAttempts = (attempts as any[]).filter((a: any) => a.course_id === course.id);
      const started      = new Set(courseAttempts.map((a: any) => a.student_id)).size;
      const completedAll = courseAttempts.filter((a: any) => a.completed_at);
      const passedAll    = completedAll.filter((a: any) => a.passed);
      const notStarted   = studentCount - started;
      const avgScore     = completedAll.length
        ? Math.round(completedAll.reduce((s: number, a: any) => s + (a.score ?? 0), 0) / completedAll.length)
        : 0;
      ...
      if (notStarted > studentCount * 0.5) flags.push(`⚠ "${course.title}": ${notStarted} students (${Math.round(notStarted/studentCount*100)}%) haven't started`);
      if (passRate < 50 && completedAll.length >= 3) flags.push(`⚠ "${course.title}": low pass rate (${passRate}%) -- questions may be too hard or lessons unclear`);
      if (avgScore < 40 && completedAll.length >= 3) flags.push(`⚠ "${course.title}": avg score ${avgScore}% -- consider reviewing content difficulty`);
    }
...
server.tool(
  ...
  'Analyse a course and return specific suggestions: questions with very high failure rates, missing explanations, lesson-less questions, and passmark calibration. The output is designed to be actioned directly.',
  {
    course_id: z.string().describe('Course ID -- call list_courses to resolve name'),
  },
  async ({ course_id }) => {
    const [courseRows, attempts] = await Promise.all([
      supabaseRun(sb => sb.from('courses').select('title, questions, passmark').eq('id', course_id).single()) as Promise<any>,
      supabaseRun(sb => sb.from('course_attempts').select('answers, score, passed').eq('course_id', course_id).not('completed_at', 'is', null)) as Promise<any[]>,
    ]);

    const course = courseRows as any;
    if (!course?.title) return { content: [{ type: 'text', text: `Course not found: ${course_id}` }] };

    const questions: any[] = course.questions ?? [];
    const completedAttempts: any[] = (attempts as any[]) ?? [];

    const suggestions: string[] = [];

    // Missing explanations
    const noExplanation = questions.filter((q: any) => !q.lessonOnly && !q.explanation?.trim());
    if (noExplanation.length) {
      ...
      suggestions.push(`  Add explanations to: ${noExplanation.slice(0, 5).map((q: any) => `"${(q.question ?? '').slice(0, 60)}"`).join(', ')}${noExplanation.length > 5 ? ` (+${noExplanation.length - 5} more)` : ''}`);
    }

    // Questions with no lesson content
    const noLesson = questions.filter((q: any) => !q.lessonOnly && !q.lesson?.body?.trim() && !q.lesson?.videoUrl);
    ...
    const vague = questions.filter((q: any) => !q.lessonOnly && (q.question ?? '').length < 20);
    if (vague.length) {
    ...
    if (completedAttempts.length >= 5) {
      ...
      if (hardQuestions.length) {
        ...
        for (const hq of hardQuestions) {
          const q = questions.find((q: any) => q.id === hq.id);
    ...
    if (completedAttempts.length >= 5) {
      const avgScore = Math.round(completedAttempts.reduce((s: number, a: any) => s + (a.score ?? 0), 0) / completedAttempts.length);
      const passRate = Math.round(completedAttempts.filter((a: any) => a.passed).length / completedAttempts.length * 100);
      if (passRate < 40) suggestions.push(`Pass rate is ${passRate}% with passmark at ${course.passmark ?? 50}% -- consider lowering the passmark or simplifying questions.`);
      if (passRate > 95 && completedAttempts.length >= 10) suggestions.push(`Pass rate is ${passRate}% -- course may be too easy. Consider raising the passmark or adding harder questions.`);
...
server.tool(
  ...
  'Find all courses and virtual experiences related to a skill or topic and create a learning path from them. Returns the matched content for review before creating.',
  {
  ...
  async ({ skill, title, description, cohort_ids, auto_create }) => {
    ...
    const [courses, ves] = await Promise.all([
      supabaseRun(sb => sb.from('courses').select('id, title, description, status')) as Promise<any[]>,
      supabaseRun(sb => sb.from('virtual_experiences').select('id, title, tagline, industry, status')) as Promise<any[]>,
    ]);

    const matchCourse = (c: any) =>
    ...
    const matchedCourses = ((courses as any[]) ?? []).filter(matchCourse);
    const matchedVEs     = ((ves     as any[]) ?? []).filter(matchVE);
    const allMatched     = [
      ...matchedCourses.map((c: any) => ({ id: c.id, title: c.title, type: 'course',  status: c.status })),
      ...matchedVEs.map((v: any)     => ({ id: v.id, title: v.title, type: 'virtual_experience', status: v.status })),
    ];
