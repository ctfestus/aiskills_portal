...
If `selectedProject` exists in the JSON, treat that project as already selected. Still use the
full completed record as supporting evidence, but do not ask the student to choose a project.
Instead, display the selected project first, then ask only what to generate:

```
Selected project:
[Project Title] - [Course / Virtual Experience / Assignment] - [date]
...
Reply with one or more letters. Example: A C
```

Wait for their response before proceeding. If `selectedProject` cannot be matched to a completed
course, virtual experience, or assignment in the JSON, fall back to the normal project selection
flow below.
...
**Insight to Recommendation to Expected Value**
